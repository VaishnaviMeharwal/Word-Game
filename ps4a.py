...
def getWordScore(word, n):
    ...
    score = 0
    for letter in word:
        for keys in SCRABBLE_LETTER_VALUES.keys():
            if letter == keys:
                score = score + SCRABBLE_LETTER_VALUES[keys]
...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList:
        for letter in word:
            if letter in keys:
                if updatedHand[letter]>=1:
                    updatedHand[letter]-=1
                    isValidLetter.append(True)
                else:
                    isValidLetter.append(False)
            else:
                isValidLetter.append(False)
    else:
        return False
...
def findCurrentHand(hand):
    currentHand = ''
    for letter in hand.keys():
        for j in range(hand[letter]):
             currentHand = currentHand + letter + " "
    return currentHand
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    currentHand = findCurrentHand(hand)
    updatedHand = hand.copy()
    # Keep track of the total score
    totalScore = 0
    # As long as there are still letters left in the hand:
    while currentHand != "":
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                currentHand = findCurrentHand(updatedHand)
